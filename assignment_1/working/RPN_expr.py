...
for expr in sys.stdin.readlines():
	...
	while k<len(stck_list):
		...
		# message - "ERROR"
			...
			if stck_list[k]=='+':
				stck.stack_push(stck.stack_pop()+ stck.stack_pop()) #stck[-2] + stck[-1]
			elif stck_list[k]=='-':
				stck.stack_push(-stck.stack_pop() + stck.stack_pop())#stck[-2] - stck[-1]
			elif stck_list[k]=='*':
				stck.stack_push(stck.stack_pop()*stck.stack_pop())#stck[-2] * stck[-1]
			elif (stck_list[k]=='/') & (stck[-1]!=0):
				stck.stack_push((1.0/(stck.stack_pop()))*stck.stack_pop())#stck[-2] / stck[-1]
			else:
