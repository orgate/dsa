...
for inp_str in sys.stdin.readlines():
	...
	if (i==offset-1)&(len(graph)!=0):
		...
		while (len(townslistdyn)>0):
			...
			islandlist = [srce]
			graph[srce][0] = 'inf'
			townslistdyn_len = len(townslistdyn)-1
			townslistdyn.pop(0)
			islanddist = []
			while (vert<len(townslistdyn)):
				ver = townslistdyn[vert][1]
				if (graph[ver][0]!='inf')&(ver!=srce):
					townslistdyn.pop(vert)
					vert-=1
					graph[ver][1] = 0
			...
			while (itwns<islandtowns):
				...
				heappush(islandcaptdist, (tot_dist, townlist[srce]))
